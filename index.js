...
async function fetctMovieBySearch(title) {
    const res = await fetch(`http://www.omdbapi.com/?apikey=f89c6c72&i=${title}`)
    const data = await res.json()
}



async function fetchMovieDetails(moviedId) {
    const res = await fetch(`http://www.omdbapi.com/?apikey=f89c6c72&i=${movieId}`)
    const data = await res.json()
    return data;
}
