...
document.querySelector('form button').addEventListener('click', async (e) => {
    ...
    try {
        const searchResponse = await fetch(`http://www.omdbapi.com/?apikey=f89c6c72&s=${movieTitle}`);
       ...
       // console.log(searchData)
        ...
        if (searchData.Search) {
            let movieDetailsPromises = searchData.Search.map(movie => {
                ...
                return fetch(`http://www.omdbapi.com/?apikey=f89c6c72&i=${movieId}`).then(res => res.json());
            });
                    ...
                    <div class="movie-list__row">
                        ...
                        <div class="movie-list__row--text__container">
                            ...
                            <div class="movie-list__row--text__info">
                                <p>${movie.Runtime}</p>
                                <p>${movie.Genre}</p>
                                <button class="movie-list__row--text__info--btn" movie-id="${movie.imdbID}">
...
document.querySelector('.movie-list__body').addEventListener('click', (e) => {
    ...
    if (e.target.closest(".movie-list__row--text__info--btn")) {
        console.log(e);
